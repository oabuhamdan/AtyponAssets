...
(function ($) {
    ...
    $.fn.Tabledit = function (options) {
                        ...
                        $td.each(function () {
                            ...
                            if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                ...
                                if (settings.columns.editable[i][2] === 'date') {
                                    var input = '<input class="tabledit-input ' + settings.inputClass + '" type="date" name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none;" disabled>';
                                } else {
                                    ...
                                    $.each(jQuery.parseJSON(settings.columns.editable[i][2].replaceAll('&quot;', '"')), function (index, value) {
                                        if (text === value) {
        ...
        function ajax(action) {
            var serialize = $table.find('.tabledit-input').serialize() + '&action=' + action + '&csrfmiddlewaretoken=' + $("input[name='csrfmiddlewaretoken']").val();
